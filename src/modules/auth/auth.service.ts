...
export class AuthService {
    ...
    private hashing(password: string): string {
    ...
    async registration(user: registUsersDto) {
        ...
        try {
            const hash = this.hashing(uid(5));
            const send = await this.emailService.sendOnEmail(
                user.email,
                "Подтверждение регистрации в gittest",
                "Подтвердите email перейдя по ссылке " +
                `http://${this.configService.get<string>("PROJECT_HOST")}/auth/${hash}`
            );
            if (send) {
                this.redis.set(hash, user.email, 3600);
                user.password = this.hashing(user.password);
    ...
    async login(user: LoginUsersDto) {
        const existUser = await this.usersService.getOneByEmail(user.email);
        if (!existUser || (await this.usersService.getPasswordByEmail(user.email) != this.hashing(user.password))) {
